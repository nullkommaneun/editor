...
export function kmeans(imageData, k=6, maxIter=12){
  ...
  for (let it=0;it<maxIter;it++){
    ...
    for (let i=0;i<points.length;i++){ const a=assign[i]; const p=points[i]; sum[a][0]+=p[0]; sum[a][1]+=p[1]; sum[a][2]+=p[2]; cnt[a]++; }
    for (let c=0;c<k;c++){ if (cnt[c]===0) continue; centroids[c][0]=sum[c][0]/cnt[c]; centroids[c][1]=sum[c][1]/cnt[c]; centroids[c][2]=sum[c][2]/cnt[c]; }
  }
...
export function labelImageByCentroids(imageData, centroids){
  ...
  for (let i=0;i<n;i++){
    ...
    const lab = rgbToLab(r,g,b);
    let best=0, bestd=Infinity;
    for (let c=0;c<centroids.length;c++){ const d = dist(lab, centroids[c]); if (d<bestd){ bestd=d; best=c; } }
    idxMap[i]=best;
  }
  return idxMap;
}
