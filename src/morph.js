...
export function maskFromClustersIdxMap(idxMap, width, height, selectedIndices=[], centroids=null, toleranceDE=0){
  ...
  if (centroids && toleranceDE>0){
    ...
    centroids.forEach((c,idx)=>{ for (const t of targets){ if (dE(c,t) <= toleranceDE){ allowed.add(idx); break; } } });
  }
...
export function closing(bin, width, height){ return erode(dilate(bin, width, height), width, height); }
export function opening(bin, width, height){ return dilate(erode(bin, width, height), width, height); }
...
export function fillHoles(bin, width, height){
  ...
  while (q.length){ const [x,y]=q.shift(); [[1,0],[-1,0],[0,1],[0,-1]].forEach(([dx,dy])=> push(x+dx,y+dy)); }
  const out = bin.slice(0);
  for (let i=0;i<out.length;i++){ if (inv[i] && !visited[i]) out[i]=1; }
  return out;
}

export function edgeRefine(bin, width, height){ return erode(bin, width, height); }
