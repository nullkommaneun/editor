  ...
  check(state, {pathMetrics=false}={}){
    ...
    if (state.grid){
      ...
      if (pathMetrics && state.startPoints?.length && state.sites?.length){
        ...
        const key=(c,r)=>`${c}_${r}`;
        const walkable2=(c,r)=> walkable(c,r);
        const astar=(sc,sr,tc,tr)=>{
          if (!walkable2(sc,sr) || !walkable2(tc,tr)) return {dist:Infinity};
          const open=new Map(); const g=new Map(); const f=new Map();
          ...
          while(open.size){
            ...
            for (const [dc,dr] of [[1,0],[-1,0],[0,1],[0,-1]]){
              ...
              if (!walkable2(nc,nr)) continue;
              const tentative = (g.get(curK)??Infinity)+1;
              if (tentative < (g.get(nk)??Infinity)){ g.set(nk,tentative); f.set(nk, tentative + h(nc,nr,tc,tr)); if (!open.has(nk)) open.set(nk,[nc,nr]); }
            }
          }
